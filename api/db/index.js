...
*/
exports.del = async (colName, query) => {
...
// 例：find('user',{name:'xiaoming'});
*/
exports.findRestrict = async (colName, query, page, num) => {
    num = num * 1;
    let index = (page - 1) * num;
    let { db, client } = await connect();

    let collection = db.collection(colName);
    let res = await collection.find(query).skip(index).limit(num).toArray();//将查询结果置为数组
    client.close();
